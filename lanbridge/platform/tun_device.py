...
class TUNDevice:
    ...
    async def _create_windows(self):
        """Create TUN via wintun.dll (from WireGuard)."""
        import ctypes
        ...
        if not wintun_path:
            # Try to download
            print("  Downloading wintun.dll...")
            wintun_path = await self._download_wintun()
            if not wintun_path:
                raise RuntimeError(
                    "Could not find wintun.dll.\n"
                    "Download from https://www.wintun.net/ and place next to lanbridge.exe"
                )
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load wintun.dll: {e}")
        ...
        # Wait a moment for the adapter to be fully ready
        ...
        if result.returncode != 0:
            logger.warning(f"netsh set address failed: {result.stderr}")
            # Try alternate method (ipv4 namespace)
            ...
            if result2.returncode != 0:
                logger.warning(f"netsh ipv4 set address also failed: {result2.stderr}")
                # Third attempt: use netsh with 'name=' prefix
                result3 = subprocess.run(
                    ['netsh', 'interface', 'ipv4', 'set', 'address',
                     f'name={self.name}', 'static', self.address, VPN_MASK],
                    capture_output=True, text=True,
                    encoding='utf-8', errors='replace'
                )
                if result3.returncode != 0:
                    logger.warning(f"netsh set address (3rd attempt) failed: {result3.stderr}")

        logger.info(f"wintun created: {self.address}/{VPN_PREFIX}")
