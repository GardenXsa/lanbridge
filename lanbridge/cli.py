...
Supports two connection modes:
  1. Direct P2P: for when you have a public IP or port forwarding
  2. Via Relay: for when you're behind VPN/NAT (works everywhere!)
...
Usage:
  ...
  lanbridge relay         -> start relay server (for VPS)
"""
...
def build_relay_code(relay_host: str, relay_port: int, session_id: str, password: str) -> str:
    """Build connection code for relay mode: R:host:port:session:password"""
    return f"R:{relay_host}:{relay_port}:{session_id}:{password}"


def parse_code(code: str):
    """Parse connection code.
    
    Direct P2P: IP:PORT:PASSWORD
    Relay mode: R:HOST:PORT:SESSION:PASSWORD
    """
    code = code.strip()
    ...
    if code.upper().startswith('R:'):
        ...
        if len(parts) < 4:
            raise ValueError(
                f"Invalid relay code: {code}\n"
                f"Format: R:RELAY_HOST:RELAY_PORT:SESSION_ID:PASSWORD"
            )
        relay_host = parts[0]
        try:
            relay_port = int(parts[1])
        except ValueError:
            raise ValueError(f"Relay port must be a number, not '{parts[1]}'")
        session_id = parts[2]
        password = ':'.join(parts[3:])  # Password may contain colons
        return ('relay', relay_host, relay_port, session_id, password)
    ...
    if len(parts) < 3:
        raise ValueError(
            f"Invalid code: {code}\n"
            f"Format: IP:PORT:PASSWORD or R:HOST:PORT:SESSION:PASSWORD"
        )
    ip = parts[0]
    try:
        port = int(parts[1])
    ...
    return ('direct', ip, port, password)
...
async def cmd_host(args):
    ...
    # Check for VPN
    from .vpn_bypass import is_vpn_active, get_vpn_info, get_real_external_ip
    vpn_info = get_vpn_info()

    if vpn_info.get('vpn_active'):
        print(f"  [!] VPN detected: {', '.join(vpn_info['vpn_interfaces'])}")
        print(f"  VPN IP: {ext_ip} (not reachable from internet)")
        print(f"  Real interface: {vpn_info.get('real_interface', 'unknown')}")
        print(f"  Real local IP: {vpn_info.get('real_local_ip', 'unknown')}")
        print()
        print(f"  [!] VPN blocks incoming connections!")
        print(f"  [!] Use relay mode instead - it works through any VPN!")
        print()
        print(f"  To create a relay room, choose option 4 (Relay mode) from the menu,")
        print(f"  or run: lanbridge host --relay")
        print()

        real_local = vpn_info.get('real_local_ip')
        if real_local:
            print(f"  Getting real external IP (bypassing VPN)...")
            real_ext_ip = get_real_external_ip(real_local)
            if real_ext_ip:
                print(f"  [+] Your real IP: {real_ext_ip}")
            else:
                print(f"  [!] Could not detect real external IP")

        real_gw = vpn_info.get('real_gateway')
        if real_gw:
            bind_ip = real_local or '0.0.0.0'

        if real_ext_ip:
            display_ip = args.ip or real_ext_ip
        else:
    ...
    behind_nat = (real_ext_ip or ext_ip) and (real_ext_ip or ext_ip) != local_ip
    if behind_nat and not vpn_info.get('vpn_active'):
        print(f"  You are behind NAT (local: {local_ip}, external: {real_ext_ip or ext_ip})")
    ...
    print()
    print("  +=====================================================+")
    print("  |            LanBridge VPN -- HOST (Direct P2P)       |")
    print("  +=====================================================+")
    ...
    if vpn_info.get('vpn_active'):
        if real_ext_ip:
            print(f"  [+] VPN bypass: using real IP {real_ext_ip}")
            print(f"  [+] Discord/other apps still work through VPN")
        else:
            print(f"  [!] VPN detected but could not find real IP")
            print(f"  [!] Direct connection likely won't work through VPN!")
            print(f"  [!] Use relay mode: lanbridge host --relay")
        print()
    elif behind_nat:
        if upnp_handler:
            ...
            print(f"  [!]   3. Or use relay mode: lanbridge host --relay")
        print()
...
async def cmd_host_relay(args):
    """Create a VPN room via relay server (works through VPN/NAT!)."""
    from .relay import generate_session_id

    password = args.password or generate_password()
    session_id = generate_session_id()
    relay_host = args.relay or DEFAULT_RELAY_HOST
    relay_port = args.relay_port or DEFAULT_RELAY_PORT
    ...
    if not is_admin():
        ...
        if sys.platform == 'win32':
            ...
            print("  [!] Root required to create virtual network adapter.")
            print("  [!] Run with sudo:")
            print(f"  [!]   sudo lanbridge host --relay")
        print()
        return False

    print()
    print("  +=====================================================+")
    print("  |         LanBridge VPN -- HOST (Relay Mode)         |")
    print("  |     Works through VPN, NAT, firewalls!             |")
    print("  +=====================================================+")
    print()
    print("  Connecting to relay server...")

    # Create VPN via relay
    from .core.vpn import run_vpn_host_relay
    ...
    def on_registered():
        print(f"  [+] Room registered on relay server!")
        print()

    try:
        tunnel, vpn, tun, transport, conn_event, reg_event = await run_vpn_host_relay(
            password=password,
        ...
        print(f"\n  [!] Failed to create VPN: {e}")
        print(f"  [!] Make sure the relay server is accessible:")
        print(f"  [!]   {relay_host}:{relay_port}")
        print()
        return False

    # Wait for registration
    try:
        await asyncio.wait_for(reg_event.wait(), timeout=10.0)
    except asyncio.TimeoutError:
        print(f"  [!] Could not register with relay server at {relay_host}:{relay_port}")
        print(f"  [!] Check your internet connection and relay server address")
        print()
        tun.stop()
        transport.close()
        return False
    ...
    print("  +=====================================================+")
    print("  |  RELAY ROOM -- Send this to your friend:            |")
    print("  |                                                     |")
    print(f"  |  Room ID:    {session_id:<37}|")
    print(f"  |  Password:   {password:<37}|")
    print("  |                                                     |")
    print("  +=====================================================+")
    print()
    ...
    relay_code = build_relay_code(relay_host, relay_port, session_id, password)
    print(f"  Connection code (one line):")
    print(f"  {relay_code}")
    print()
    print(f"  Or your friend runs:")
    print(f"  {exe_name} connect {relay_code}")
    print()
    print(f"  Waiting for friend... (Ctrl+C to stop)")
    print()
...
async def cmd_connect(args):
    """Connect to a VPN room (auto-detects direct or relay mode)."""
    try:
    ...
    if mode == 'relay':
        _, relay_host, relay_port, session_id, password = parsed
        return await _connect_relay(relay_host, relay_port, session_id, password)
    else:
        _, ip, port, password = parsed
        return await _connect_direct(ip, port, password)


async def _connect_direct(ip: str, port: int, password: str):
    """Connect to a host directly (P2P)."""
    bind_ip = '0.0.0.0'

    # Check for VPN
    from .vpn_bypass import get_vpn_info, add_bypass_route, remove_bypass_route
    vpn_info = get_vpn_info()
    ...
    if vpn_info.get('vpn_active'):
        ...
        if real_local:
            bind_ip = real_local
            print(f"  [+] Bypassing VPN: UDP traffic through {bind_ip}")

        if real_gw:
            route_added = add_bypass_route(0, ip)
            if route_added:
                print(f"  [+] Route added: {ip} -> {real_gw} (bypassing VPN)")
    ...
    print()
    print("  +=====================================================+")
    print("  |            LanBridge VPN -- CLIENT (Direct)         |")
    print("  +=====================================================+")
    ...
    try:
        ...
        if vpn_info.get('vpn_active'):
            remove_bypass_route(0, ip)
            print("  VPN bypass route removed.")
        print("\n  VPN stopped.")

    return True


async def _connect_relay(relay_host: str, relay_port: int,
                          session_id: str, password: str):
    """Connect to a host via relay server."""
    # Check admin rights
    if not is_admin():
        print()
        if sys.platform == 'win32':
    ...
    print()
    print("  +=====================================================+")
    print("  |         LanBridge VPN -- CLIENT (Relay Mode)        |")
    print("  |       Works through VPN, NAT, firewalls!            |")
    print("  +=====================================================+")
    print(f"  |  Relay:      {relay_host}:{relay_port:<29}|")
    print(f"  |  Room ID:    {session_id:<37}|")
    print(f"  |  Password:   {password:<37}|")
    print("  |                                                     |")
    print(f"  |  Your virtual IP:  {CLIENT_IP:<30}|")
    ...
    # Create VPN via relay
    from .core.vpn import run_vpn_client_relay
    ...
    try:
        tunnel, vpn, tun, transport, conn_event = await run_vpn_client_relay(
            password=password,
            relay_addr=relay_addr,
            session_id=session_id,
            on_connected=on_connected,
    ...
    print(f"  Virtual interface: {tun.name}")
    print(f"  IP: {CLIENT_IP}")
    print(f"  Connecting to host via relay...")
    print(f"  (Ctrl+C to disconnect)")
    print()
...
async def cmd_relay(args):
    ...
    print(f"  Cannot read traffic - everything is end-to-end encrypted!")
    print()
...
def _print_troubleshooting(mode: str, port: int, ip: str = None):
    ...
    if mode == 'host':
        ...
        print(f"  4. If you're behind VPN, use relay mode: lanbridge host --relay")
    else:
        ...
        print(f"  4. If host is behind VPN, use relay mode instead")
    print()
...
def _interactive_menu():
    ...
    while True:
        ...
        print("  |  2. Create a room via Relay (VPN/NAT safe!)         |")
        print("  |  3. Connect to a room                               |")
        print("  |  4. Game connection hints                           |")
        print("  |  0. Exit                                            |")
        print("  |                                                     |")
        print("  +=====================================================+")
        print()
...
def _run_host_relay_interactive():
    """Run host relay command from interactive menu."""
    print()
    ...
    if not is_admin():
        if sys.platform == 'win32':
            ...
            print("  [!] Root required!")
            print("  [!] Run with: sudo lanbridge host --relay")
        print()
        input("  Press Enter to continue...")
        return

    args = argparse.Namespace(
        password=None, relay=None, relay_port=None, verbose=False
    )
    ...
    try:
        asyncio.run(cmd_host_relay(args))
    except KeyboardInterrupt:
        print("\n  VPN stopped by user.")
    except Exception as e:
        print(f"\n  [!] Error: {e}")
...
def _run_connect_interactive():
    ...
    print("  Direct:    IP:PORT:PASSWORD")
    print("  Via Relay: R:HOST:PORT:SESSION:PASSWORD")
    print("  Example:   192.168.1.5:9876:MyPassword123")
    print("  Example:   R:relay.lanbridge.io:9876:a3f2b1c0:MyPassword123")
    print()
...
def main():
    ...
    hp.add_argument('--relay', type=str, default=None,
                    help='Relay server hostname (enables relay mode)')
    hp.add_argument('--relay-port', type=int, default=None,
                    help='Relay server port (default: 9876)')

    # connect
    cp = sub.add_parser('connect', help='Connect to a VPN room')
    cp.add_argument('code', type=str,
                    help='Connection code: IP:PORT:PASSWORD or R:HOST:PORT:SESSION:PASSWORD')

    # games
    sub.add_parser('games', help='Game connection hints')

    # relay
    rp = sub.add_parser('relay', help='Start relay server (for VPS)')
