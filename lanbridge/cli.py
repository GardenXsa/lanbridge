...
async def cmd_host(args):
    ...
    # Detect IP (through VPN if active)
    print("  Detecting IP...")
    ext_ip = get_external_ip()
    local_ip = get_local_ip()

    # Check for VPN and try to bypass
    from .vpn_bypass import is_vpn_active, get_vpn_info, get_real_external_ip, add_bypass_route, remove_bypass_route
    ...
    if vpn_info.get('vpn_active'):
        ...
        # Try to get the real external IP (from ISP, not VPN)
        ...
        # Add bypass route for UDP traffic
        real_gw = vpn_info.get('real_gateway')
        if real_gw:
            print(f"  Adding route to bypass VPN...")
            # Bind UDP socket to real interface
            bind_ip = real_local or '0.0.0.0'
            print(f"  [+] UDP traffic will go through: {bind_ip} (real interface)")
    ...
    if vpn_info.get('vpn_active'):
        if real_ext_ip:
            ...
            print(f"  [!] If friend can't connect, try:")
            print(f"  [!]   1. Disable VPN temporarily")
            print(f"  [!]   2. Enable split tunneling in VPN settings")
            print(f"  [!]   3. Add lanbridge.exe to VPN exclusion list")
        print()
    elif behind_nat:
        if upnp_handler:
...
async def cmd_connect(args):
    ...
    # Check for VPN and add bypass route to host's IP
    from .vpn_bypass import get_vpn_info, get_real_external_ip, add_bypass_route, remove_bypass_route
    vpn_info = get_vpn_info()
    ...
    if vpn_info.get('vpn_active'):
        ...
        if real_local:
            # Bind UDP socket to real interface to bypass VPN
            bind_ip = real_local
            print(f"  [+] Bypassing VPN: UDP traffic through {bind_ip}")

        # Add route to host's IP through real gateway
        if real_gw:
